...
parameters = dict(powerSupply={'type': 'value', 'value': 1, 'label': 'Power Supply',
                               'min': 0, 'max': 10},
                  powerSupply1={'type': 'value', 'value': 0, 'label': 'NOT CONNECTED - 1',
                                'min': 0, 'max': 10},
                  randomText={'type': 'text', 'label': 'Something', 'text': 'hello'},
                  interlockActive={'type': 'state', 'label': 'Interlock Active', 'state': True})
...
class ParameterTemplate(QtGui.QWidget):
    ...
    def checkstateChanged(self, state):
        parameters[self.name]['state'] = state
        print parameters
